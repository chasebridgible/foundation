...
function usage() {
  return `Usage:
  npm run foundation:capability-map:check -- --repo /path/to/repo --run-id YYYYMMDD-NN [--phase batch|handoff] [--report path] [--json] [--no-write]
...
function main() {
  ...
  const check = validateCapabilityMap({ repoRoot, runId, outDir, phase, reportPath });
  const summary = summarizeResults(check.results);
  const payload = {
    schema: "foundation.backfill.capability-map-check.v1",
    runId,
    phase,
    generatedAt: new Date().toISOString(),
    surfaceFunctionMapPath: path.relative(repoRoot, check.surfaceFunctionMapPath || ""),
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    artifactsRead: [payload.surfaceFunctionMapPath, payload.registryPath].filter(Boolean),
    artifactsChanged: options["no-write"] ? [] : [path.relative(repoRoot, checkPath)],
    commands: ["foundation:capability-map:check"],
    checks: [{ name: "capability-map-check", result: summary.fail === 0 ? "passed" : "failed" }],
