...
import {
  appendRunLogEvent,
  checkPathFor as artifactInventoryCheckPathFor,
  defaultBackfillDir,
  ensureDir,
  evalReceiptPathFor as artifactInventoryEvalReceiptPathFor,
  ...
  readJson,
  readJsonl,
  registryPathFor as artifactInventoryPathFor,
  renderResultsText,
  summarizeResults,
  writeJson,
  writeJsonl
...
function surfaceFunctionMapPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `surface-function-map-${runId}.jsonl`);
}

function surfaceCheckPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `surface-function-map-check-${runId}.json`);
}

function surfaceEvalReceiptPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `surface-function-map-eval-${runId}.jsonl`);
}

function surfaceEvalSummaryPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `surface-function-map-eval-summary-${runId}.html`);
}

function surfaceRefreshPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `surface-function-map-refresh-${runId}.json`);
}
...
function surfaceFunctionMapScopeDecision(fileRow) {
...
function isSurfaceFunctionMapEligibleRow(fileRow) {
  return surfaceFunctionMapScopeDecision(fileRow).eligible;
}

function surfaceFunctionMapEligibleRows(fileRows) {
  return fileRows.filter(isSurfaceFunctionMapEligibleRow);
}

function surfaceFunctionMapScopeCounts(fileRows) {
  const eligible = [];
  const skipped = [];
  for (const fileRow of fileRows) {
    const decision = surfaceFunctionMapScopeDecision(fileRow);
...
function createPendingSurfaceRow(fileRow) {
  return {
    schema: "foundation.backfill.surface-function-map-row.v1",
    runId: fileRow.runId,
...
function markSurfaceRowsForFile({ fileRows, surfaceRows, filePath, surfaceSpecs }) {
  ...
  const scope = surfaceFunctionMapScopeDecision(fileRow);
  if (!scope.eligible) {
...
function nextSurfaceFunctionMapTarget({ fileRows, surfaceRows }) {
  ...
  const candidates = surfaceRows
    ...
    .filter(candidate => isSurfaceFunctionMapEligibleRow(candidate.fileRow))
    .sort((left, right) => {
...
function readArtifactInventoryRows(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  const registryPath = artifactInventoryPathFor(repoRoot, runId, outDir);
  const parsed = readJsonl(registryPath);
  return { registryPath, ...parsed };
}

function readArtifactInventoryEvalSummary(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  const receiptPath = artifactInventoryEvalReceiptPathFor(repoRoot, runId, outDir);
...
function validateArtifactInventoryHandoff(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  const results = [];
  const registry = readArtifactInventoryRows(repoRoot, runId, outDir);
  if (registry.errors.length > 0) {
    results.push(fail("upstream-artifact-inventory-jsonl", "Artifact Inventory JSONL must parse before surface extraction", { errors: registry.errors }));
  } else {
    results.push(pass("upstream-artifact-inventory-jsonl", "Artifact Inventory JSONL parses"));
  }

  const checkPath = artifactInventoryCheckPathFor(repoRoot, runId, outDir);
  if (!fs.existsSync(checkPath)) {
    results.push(fail("upstream-artifact-inventory-check", "Passing Artifact Inventory check artifact is required before surface extraction"));
  } else {
    const check = readJson(checkPath);
    results.push(check?.summary?.fail === 0
      ? pass("upstream-artifact-inventory-check", "Artifact Inventory check artifact passes")
      : fail("upstream-artifact-inventory-check", "Artifact Inventory check artifact must pass", { summary: check?.summary || null }));
  }

  const evalSummary = readArtifactInventoryEvalSummary(repoRoot, runId, outDir);
  results.push(evalSummary?.acceptabilityGate?.acceptable
    ? pass("upstream-artifact-inventory-eval", "Artifact Inventory eval artifact passes")
    : fail("upstream-artifact-inventory-eval", "Passing Artifact Inventory eval receipt is required before surface extraction"));

  const pending = registry.rows.filter(row => row.status !== "mapped").map(row => row.path);
  results.push(pending.length === 0
    ? pass("upstream-artifact-inventory-mapped", "All upstream Artifact Inventory rows are mapped")
    : fail("upstream-artifact-inventory-mapped", "Surface extraction requires mapped upstream file rows", { pending }));

  return { registry, results };
}

function createInitialSurfaceRows(fileRows) {
  return surfaceFunctionMapEligibleRows(fileRows).map(createPendingSurfaceRow);
}
...
function surfaceQualityWarnings({ repoRoot, fileRows, rowsByUpstream, phase = "handoff" }) {
  ...
  for (const fileRow of fileRows.filter(isSurfaceFunctionMapEligibleRow)) {
...
function validateSurfaceRowShape(row, prefix, results) {
  if (row?.schema !== "foundation.backfill.surface-function-map-row.v1") {
...
function validateSurfaceRows({ repoRoot = null, fileRows, surfaceRows, phase = "handoff" }) {
  ...
  for (const [index, row] of surfaceRows.entries()) {
    ...
    for (const fileId of row.upstreamFileIds || []) {
      ...
      if (upstream && !isSurfaceFunctionMapEligibleRow(upstream)) {
        outOfScopeRows.push({ surfaceId: row.surfaceId, fileId, path: upstream.path, reason: surfaceFunctionMapScopeDecision(upstream).reason });
      }
    }
...
function validateSurfaceReportState({ repoRoot, runId, outDir, reportPath, fileRows, surfaceRows }) {
  ...
  const state = parseJsonScript(html, "backfill-surface-function-map-state");
  if (!state) return [fail("surface-report-state", "Report is missing backfill-surface-function-map-state JSON script")];
  ...
  const expected = {
    registryPath: path.relative(repoRoot, surfaceFunctionMapPathFor(repoRoot, runId, outDir)),
    ...
    eligibleFileCount: surfaceFunctionMapScopeCounts(fileRows).eligible.length,
    skippedFileCount: surfaceFunctionMapScopeCounts(fileRows).skipped.length,
    pendingCount,
    needsEvidenceCount,
...
function validateSurfaceFunctionMap({ repoRoot, runId, outDir = defaultBackfillDir(repoRoot), phase = "handoff", reportPath = null }) {
  const results = [];
  const upstream = validateArtifactInventoryHandoff(repoRoot, runId, outDir);
  ...
  if (!fs.existsSync(surfacePath)) {
    return {
      ...
      artifactInventoryPath: upstream.registry.registryPath,
      fileRows: upstream.registry.rows,
      surfaceRows: [],
      results: [...results, fail("surface-function-map-exists", `Surface / Function Map does not exist: ${surfacePath}`)]
    };
  }
  const parsed = readJsonl(surfacePath);
  results.push(pass("surface-function-map-exists", "Surface / Function Map exists"));
...
function mergeSurfaceRowsForRefresh({ fileRows, existingSurfaceRows }) {
  const fileById = new Map(fileRows.map(row => [row.fileId, row]));
  const eligibleRows = surfaceFunctionMapEligibleRows(fileRows);
  ...
  output.sort((left, right) => left.upstreamPaths[0].localeCompare(right.upstreamPaths[0]) || left.surfaceId.localeCompare(right.surfaceId));
  return { rows: output, changed, removed, skipped: surfaceFunctionMapScopeCounts(fileRows).skipped };
}
...
export {
  ...
  defaultBackfillDir,
  ensureDir,
  isSurfaceFunctionMapEligibleRow,
  markSurfaceRowsForFile,
  mergeSurfaceRowsForRefresh,
  nextSurfaceFunctionMapTarget,
  parseCliArgs,
  ...
  summarizeResults,
  surfaceFunctionMapEligibleRows,
  surfaceFunctionMapScopeCounts,
  surfaceFunctionMapScopeDecision,
  surfaceCheckPathFor,
  surfaceEvalReceiptPathFor,
  surfaceEvalSummaryPathFor,
  surfaceRefreshPathFor,
  surfaceFunctionMapPathFor,
  validateArtifactInventoryHandoff,
