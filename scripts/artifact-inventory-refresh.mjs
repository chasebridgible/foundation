...
function usage() {
  return `Usage:
  npm run foundation:artifact-inventory:refresh -- --repo /path/to/repo --run-id YYYYMMDD-NN [--fill-changed] [--out-dir path] [--run-log path]
...
function main() {
  ...
  const payload = {
    schema: "foundation.backfill.artifact-inventory-refresh.v1",
    runId,
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    commands: ["foundation:artifact-inventory:refresh"],
    checks: [],
