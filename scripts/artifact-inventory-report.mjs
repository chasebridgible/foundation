...
function usage() {
  return `Usage:
  npm run foundation:artifact-inventory:report -- --repo /path/to/repo --run-id YYYYMMDD-NN [--report docs/specs/backfill/review-report-YYYYMMDD-NN.html] [--run-log path]

Creates or updates a target backfill report with the Artifact Inventory V1 handoff state.`;
}
...
function buildState({ repoRoot, runId, outDir, runLogPath }) {
  ...
  return {
    schema: "foundation.backfill.artifact-inventory-report-state.v1",
    runId,
    ...
    nextLayer: pendingCount === 0 && blockingFlagCount === 0 && check?.summary?.fail === 0 && evalSummary?.acceptabilityGate?.acceptable
      ...
      : "Artifact Inventory revision"
  };
}

function renderSection(state) {
  return `<section id="artifact-inventory-v1-state" data-spec-section="artifact-inventory-v1-state" data-section-type="handoff">
  <h2>Artifact Inventory V1 State</h2>
...
<title>Artifact Inventory Report ${state.runId}</title>
<link rel="stylesheet" href="../spec-system.css">
<script type="application/json" id="backfill-artifact-inventory-state">
...
<main class="main">
  <section id="summary">
    ...
    <p class="lede">Minimal report generated by the Foundation Artifact Inventory V1 report integration.</p>
  </section>
...
function upsertReportState(html, state) {
  const script = `<script type="application/json" id="backfill-artifact-inventory-state">\n${JSON.stringify(state, null, 2)}\n</script>`;
  const section = renderSection(state);
  let nextHtml = html;
  if (/<script\b[^>]*id=["']backfill-artifact-inventory-state["'][^>]*>[\s\S]*?<\/script>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<script\b[^>]*id=["']backfill-artifact-inventory-state["'][^>]*>[\s\S]*?<\/script>/i, script);
  } else {
    nextHtml = nextHtml.replace("</head>", `${script}\n</head>`);
  }

  if (/<section\b[^>]*id=["']artifact-inventory-v1-state["'][^>]*>[\s\S]*?<\/section>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<section\b[^>]*id=["']artifact-inventory-v1-state["'][^>]*>[\s\S]*?<\/section>/i, section);
