...
const capabilityRefreshScript = path.join(scriptsDir, "capability-map-refresh.mjs");
const capabilityReportScript = path.join(scriptsDir, "capability-map-report.mjs");

function makeRepo() {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foundation-capability-map-"));
...
function prepareSurfaceFunctionMap(repoRoot, runId = "20260529-01", runLog = null) {
...
function prepareCapabilityMap(repoRoot, runId = "20260529-01", runLog = null) {
...
test("init requires passing Surface / Function Map handoff and creates pending capability rows", () => {
  ...
  assert.match(output, /capability-map-skeleton/);
  const rows = readJsonl(capabilityMapPathFor(repoRoot, runId)).rows;
...
test("init rejects Surface / Function Map handoff with unresolved eval revision targets", () => {
  ...
  const runId = "20260529-01";
  prepareSurfaceFunctionMap(repoRoot, runId);
  const receiptPath = path.join(defaultBackfillDir(repoRoot), `surface-function-map-eval-${runId}.jsonl`);
...
test("checker rejects pending rows at handoff and allows them during batch phase", () => {
  ...
  const handoff = validateCapabilityMap({ repoRoot, runId, phase: "handoff" });
  assert.equal(hasFailure(handoff.results, "handoff-no-pending-capabilities"), true);
  const batch = validateCapabilityMap({ repoRoot, runId, phase: "batch" });
...
test("fill --next names the next pending surface target without mutating rows", () => {
  ...
  const runId = "20260529-01";
  prepareCapabilitySkeleton(repoRoot, runId);
  const before = readJsonl(capabilityMapPathFor(repoRoot, runId)).rows;
  const output = runNode(capabilityFillScript, ["--repo", repoRoot, "--run-id", runId, "--next"], repoRoot);
  const payload = JSON.parse(output);
  const after = readJsonl(capabilityMapPathFor(repoRoot, runId)).rows;
...
test("checker fails stale upstream refs, missing coverage, and needs-split rows without split criteria", () => {
  ...
  const results = validateCapabilityMap({ repoRoot, runId }).results;
...
test("report command embeds matrix state and checker can detect report drift", () => {
  ...
  prepareCapabilityMap(repoRoot, runId, runLog);
  ...
  const drift = validateCapabilityMap({ repoRoot, runId, reportPath }).results;
...
test("check command writes check artifact", () => {
  ...
  assert.equal(check.schema, "foundation.backfill.capability-map-check.v1");
