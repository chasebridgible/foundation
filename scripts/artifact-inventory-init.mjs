...
function usage() {
  return `Usage:
  npm run foundation:artifact-inventory:init -- --repo /path/to/repo --run-id YYYYMMDD-NN [--mode backfill|steady-state] [--out-dir path] [--run-log path]
...
function main() {
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    commands: ["foundation:artifact-inventory:init"],
