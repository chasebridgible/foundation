...
const initScript = path.join(scriptsDir, "artifact-inventory-init.mjs");
const fillScript = path.join(scriptsDir, "artifact-inventory-fill.mjs");
const checkScript = path.join(scriptsDir, "artifact-inventory-check.mjs");
const evalScript = path.join(scriptsDir, "artifact-inventory-eval.mjs");
const refreshScript = path.join(scriptsDir, "artifact-inventory-refresh.mjs");
const reportScript = path.join(scriptsDir, "artifact-inventory-report.mjs");

function makeRepo() {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foundation-artifact-inventory-"));
