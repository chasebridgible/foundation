...
function usage() {
  return `Usage:
  npm run foundation:artifact-inventory:check -- --repo /path/to/repo --run-id YYYYMMDD-NN [--mode bootstrap|strict] [--phase batch|handoff] [--json] [--no-write]
...
function main() {
  ...
  const payload = {
    schema: "foundation.backfill.artifact-inventory-check.v1",
    runId,
    mode,
    phase,
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    commands: ["foundation:artifact-inventory:check"],
    checks: [{ name: "artifact-inventory-check", result: summary.fail === 0 ? "passed" : "failed" }],
