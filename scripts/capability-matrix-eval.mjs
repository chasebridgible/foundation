...
function main() {
  ...
  const check = validateCapabilityMatrix({ repoRoot, runId, outDir, phase: "handoff" });
  ...
  const summaryReceipt = {
    schema: "foundation.backfill.capability-matrix-eval.v1",
    receiptType: "summary",
    runId,
    sequence: 1,
  ...
  fs.writeFileSync(summaryPath, renderHtmlSummary({
    runId,
    repoRoot,
    registryPath: capabilityMatrixPathFor(repoRoot, runId, outDir),
    receiptPath,
    summaryPath,
    sampleRows,
    aggregate,
