...
function usage() {
  return `Usage:
  npm run foundation:artifact-inventory:eval -- --repo /path/to/repo --run-id YYYYMMDD-NN [--sample all] [--mode bootstrap|strict] [--run-log path]
...
function main() {
  ...
  const summaryReceipt = {
    schema: "foundation.backfill.artifact-inventory-eval.v1",
    receiptType: "summary",
    runId,
    sequence: 1,
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    commands: ["foundation:artifact-inventory:eval"],
    checks: [{ name: "artifact-inventory-eval", result: aggregate.acceptable ? "passed" : "failed" }],
