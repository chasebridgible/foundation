...
function usage() {
  return `Usage:
  npm run foundation:surface-function-map:report -- --repo /path/to/repo --run-id YYYYMMDD-NN [--report docs/specs/backfill/review-report-YYYYMMDD-NN.html] [--run-log path]
...
function buildState({ repoRoot, runId, outDir, runLogPath }) {
  ...
  const scope = surfaceFunctionMapScopeCounts(artifactInventory.rows);
  ...
  return {
    schema: "foundation.backfill.surface-function-map-report-state.v1",
    runId,
...
function renderSection(state) {
  return `<section id="surface-function-map-v1-state" data-spec-section="surface-function-map-v1-state" data-section-type="handoff">
...
<script type="application/json" id="backfill-surface-function-map-state">
...
function upsertReportState(html, state) {
  const script = `<script type="application/json" id="backfill-surface-function-map-state">\n${JSON.stringify(state, null, 2)}\n</script>`;
  const section = renderSection(state);
  let nextHtml = html;
  if (/<script\b[^>]*id=["']backfill-surface-function-map-state["'][^>]*>[\s\S]*?<\/script>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<script\b[^>]*id=["']backfill-surface-function-map-state["'][^>]*>[\s\S]*?<\/script>/i, script);
  } else {
    nextHtml = nextHtml.replace("</head>", `${script}\n</head>`);
  }

  if (/<section\b[^>]*id=["']surface-function-map-v1-state["'][^>]*>[\s\S]*?<\/section>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<section\b[^>]*id=["']surface-function-map-v1-state["'][^>]*>[\s\S]*?<\/section>/i, section);
...
function main() {
  ...
  appendRunLogEvent(runLogPath, {
    ...
    commands: ["foundation:surface-function-map:report"],
    checks: [{ name: "surface-function-map-handoff", result: state.nextLayer === "Capability Map" ? "passed" : "needs-revision" }],
