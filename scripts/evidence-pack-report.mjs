...
Creates or updates a target backfill report with Evidence Pack handoff state and canonical Evidence Pack JSON.`;
}
...
function renderSection(state) {
  ...
  <h2>Evidence Pack V1 State</h2>
  <table class="status-table">
    <tbody>
      <tr><td>Evidence Pack</td><td><code>${escapeHtml(state.packPath)}</code></td></tr>
      ...
      <tr><td>Evidence packs</td><td>${state.packCount}</td></tr>
      ...
      <tr><td>Ready-for-flow rows</td><td>${state.readyForFlowCount}</td></tr>
  ...
  <p>Flow Extraction may only consume this layer when the next layer is <code>flow extraction</code>; otherwise rows named by checker or eval receipts must return to the Evidence Pack fill loop.</p>
</section>`;
}

function renderPackRows(payload) {
  const rows = payload.packs.length === 0
    ? "<tr><td colspan=\"7\">No Evidence Pack rows.</td></tr>"
...
<title>Evidence Pack Report ${escapeHtml(state.runId)}</title>
...
<main class="main">
  <section id="summary">
    ...
    <p class="lede">Minimal report generated by the Foundation Evidence Pack V1 report integration.</p>
  </section>
...
function main() {
  ...
  appendRunLogEvent(runLogPath, {
    ...
    phase: "handoff",
    event: "handoff",
    summary: `Evidence Pack handoff recorded with next layer ${state.nextLayer}.`,
    artifactsRead: [
      state.packPath,
      state.checkerPath,
    ...
    checks: [{ name: "evidence-pack-handoff", result: state.nextLayer === "flow extraction" ? "passed" : "needs-revision" }],
    result: state.nextLayer === "flow extraction"
      ? "Evidence Pack handoff gate passed."
      : "Evidence Pack handoff gate did not pass.",
    nextAction: state.nextLayer === "flow extraction"
      ? "Flow Extraction may consume Evidence Pack artifacts."
      : "Revise Evidence Pack rows named by checker or eval receipts."
  });
