...
function edgeTypeForRelationship(relationship) {
...
export function createDefaultGraphMetadata(metadata, sectionsInput = []) {
  ...
  if (metadata.parent) {
    edges.push({
      id: `edge:${rootId}:depends-on:${specNodeId(metadata.parent)}`,
      type: metadata.type === "eval" ? "evaluates" : "depends-on",
      from: rootId,
  ...
  for (const child of metadata.children || []) {
    edges.push({
      id: `edge:${rootId}:contains:${specNodeId(child)}`,
      type: "contains",
      from: rootId,
      to: specNodeId(child),
      source
  ...
  for (const related of metadata.relatedSpecs || []) {
    ...
    const type = metadata.type === "template" ? "templates" : edgeTypeForRelationship(related.relationship);
    edges.push({
