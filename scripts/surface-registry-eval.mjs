...
function main() {
  ...
  const check = validateSurfaceRegistry({ repoRoot, runId, outDir, phase: "handoff" });
  ...
  const summaryReceipt = {
    schema: "foundation.backfill.surface-registry-eval.v1",
    receiptType: "summary",
    runId,
    sequence: 1,
  ...
  fs.writeFileSync(summaryPath, renderHtmlSummary({
    runId,
    repoRoot,
    registryPath: surfaceRegistryPathFor(repoRoot, runId, outDir),
    receiptPath,
    summaryPath,
    sampleRows,
    aggregate,
