...
function usage() {
  return `Usage:
  npm run foundation:capability-map:init -- --repo /path/to/repo --run-id YYYYMMDD-NN [--out-dir path] [--report path] [--run-log path]
...
function main() {
  ...
  const handoff = validateSurfaceFunctionMapHandoff(repoRoot, runId, outDir, reportPath);
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    artifactsRead: [path.relative(repoRoot, handoff.surfaceFunctionMapPath)],
    artifactsChanged: [path.relative(repoRoot, matrixPath)],
    commands: ["foundation:capability-map:init"],
    checks: [{ name: "surface-function-map-handoff", result: "passed" }],
    durationSeconds: 0,
  ...
  console.log(renderResultsText("Capability Map init", [
    ...handoff.results,
    { id: "capability-map-skeleton", status: "pass", message: `Created ${rows.length} pending capability row(s)` }
  ]));
}
