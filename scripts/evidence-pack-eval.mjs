...
function main() {
  ...
  const check = validateEvidencePack({ repoRoot, runId, outDir, phase: "handoff", skipEvalFreshness: true });
  const queueById = new Map(check.queueRows.map(row => [row.sliceId, row]));
  const sampleRows = selectEvidencePackEvalSample(check.packRows, options.sample || "risk");
  const rowReceipts = sampleRows.map(row => scoreEvidencePackRow(row, queueById));
  const aggregate = aggregateEvidencePackEval(check.results, rowReceipts);
  ...
  const receiptPath = evidencePackEvalReceiptPathFor(repoRoot, runId, outDir);
  const summaryPath = evidencePackSummaryPathFor(repoRoot, runId, outDir);
  const summaryReceipt = {
    schema: "foundation.backfill.evidence-pack-eval.v1",
    receiptType: "summary",
    runId,
    sequence: 1,
