...
import { registryPathFor } from "./file-registry-core.mjs";
...
const evidenceInitScript = path.join(scriptsDir, "evidence-pack-init.mjs");
const evidenceFillScript = path.join(scriptsDir, "evidence-pack-fill.mjs");
const evidenceCheckScript = path.join(scriptsDir, "evidence-pack-check.mjs");
const evidenceEvalScript = path.join(scriptsDir, "evidence-pack-eval.mjs");
const evidenceRefreshScript = path.join(scriptsDir, "evidence-pack-refresh.mjs");
const evidenceReportScript = path.join(scriptsDir, "evidence-pack-report.mjs");

function makeRepo() {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foundation-evidence-pack-"));
...
function preparePassingSplitQueueHandoff(repoRoot, runId = "20260529-01") {
...
function surfaceIdByPath(repoRoot, runId, filePath) {
  return readJsonl(surfaceRegistryPathFor(repoRoot, runId)).rows.find(row => row.upstreamPaths?.[0] === filePath)?.surfaceId;
}
...
function preparePassingEvidencePack(repoRoot, runId = "20260529-01", runLog = null) {
  preparePassingSplitQueueHandoff(repoRoot, runId);
  runNode(evidenceInitScript, ["--repo", repoRoot, "--run-id", runId], repoRoot);
  const slices = readJsonl(splitQueuePathFor(repoRoot, runId)).rows;
...
test("init requires current Define Spec Jobs eval with resolved revision targets and creates pending packs", () => {
  ...
  assert.match(output, /evidence-pack-skeleton/);
  const rows = readJsonl(evidencePackPathFor(repoRoot, runId)).rows;
  ...
  const receiptPath = splitQueueEvalReceiptPathFor(repoRoot, runId);
...
test("fill --next is read-only and fill rejects coarse shortcuts", () => {
  ...
  preparePassingSplitQueueHandoff(repoRoot, runId);
  runNode(evidenceInitScript, ["--repo", repoRoot, "--run-id", runId], repoRoot);
  const before = readJsonl(evidencePackPathFor(repoRoot, runId)).rows;
  const output = runNode(evidenceFillScript, ["--repo", repoRoot, "--run-id", runId, "--next"], repoRoot);
  const payload = JSON.parse(output);
  const after = readJsonl(evidencePackPathFor(repoRoot, runId)).rows;
  assert.equal(payload.schema, "foundation.backfill.evidence-pack-next-target.v1");
  assert.equal(typeof payload.target.packId, "string");
  assert.deepEqual(after, before);

  const sliceId = readJsonl(splitQueuePathFor(repoRoot, runId)).rows[0].sliceId;
  assert.throws(
...
test("filled packs pass check, eval writes receipts, and report drift is detected", () => {
  ...
  preparePassingEvidencePack(repoRoot, runId, runLog);
  ...
  const receipts = readJsonl(evidencePackEvalReceiptPathFor(repoRoot, runId));
  ...
  assert.equal(fs.existsSync(evidencePackSummaryPathFor(repoRoot, runId)), true);
...
test("checker blocks stale eval receipts after pack changes and refresh invalidates stale upstream slices", () => {
  const repoRoot = makeRepo();
  const runId = "20260529-01";
  preparePassingEvidencePack(repoRoot, runId);
  ...
  const packPath = evidencePackPathFor(repoRoot, runId);
  ...
  writeJsonl(packPath, rows.map((row, index) => index === 0
    ...
    : row));
  const staleEval = validateEvidencePack({ repoRoot, runId }).results;
  assert.equal(hasFailure(staleEval, "evidence-pack-eval-current"), true);
  ...
  runNode(evidenceEvalScript, ["--repo", repoRoot, "--run-id", runId, "--sample", "all"], repoRoot);
  const refreshedEval = validateEvidencePack({ repoRoot, runId }).results;
  assert.equal(hasFailure(refreshedEval, "evidence-pack-eval-current"), false);
  ...
  const staleUpstream = validateEvidencePack({ repoRoot, runId }).results;
  assert.equal(hasFailure(staleUpstream, "evidence-pack-upstream-fresh"), true);
  const output = runNode(evidenceRefreshScript, ["--repo", repoRoot, "--run-id", runId], repoRoot);
  const payload = JSON.parse(output);
  assert.equal(payload.changedCount > 0, true);
  const refreshedRows = readJsonl(evidencePackPathFor(repoRoot, runId)).rows;
...
test("check command writes check artifact", () => {
  ...
  preparePassingEvidencePack(repoRoot, runId);
  runNode(evidenceCheckScript, ["--repo", repoRoot, "--run-id", runId], repoRoot);
  const check = readJson(evidencePackCheckPathFor(repoRoot, runId));
  assert.equal(check.schema, "foundation.backfill.evidence-pack-check.v1");
