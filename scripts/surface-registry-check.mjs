...
function main() {
  ...
  const check = validateSurfaceRegistry({ repoRoot, runId, outDir, phase, reportPath });
  const summary = summarizeResults(check.results);
  const payload = {
    schema: "foundation.backfill.surface-registry-check.v1",
    runId,
    phase,
    generatedAt: new Date().toISOString(),
    fileRegistryPath: path.relative(repoRoot, check.fileRegistryPath || ""),
