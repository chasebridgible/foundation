...
const surfaceRefreshScript = path.join(scriptsDir, "surface-function-map-refresh.mjs");
const surfaceReportScript = path.join(scriptsDir, "surface-function-map-report.mjs");

function makeRepo() {
  const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "foundation-surface-function-map-"));
  ...
  fs.writeFileSync(path.join(repoRoot, "package-lock.json"), JSON.stringify({ name: "surface-function-map-test", lockfileVersion: 3 }, null, 2), "utf8");
...
function prepareSurfaceFunctionMap(repoRoot, runId = "20260527-01", runLog = null) {
...
test("init requires passing Artifact Inventory handoff and creates pending surface rows", () => {
  ...
  assert.match(output, /surface-function-map-skeleton/);
  const surfaces = readJsonl(surfaceFunctionMapPathFor(repoRoot, runId));
  const fileRows = readJsonl(path.join(defaultBackfillDir(repoRoot), `artifact-inventory-${runId}.jsonl`)).rows;
  const scope = surfaceFunctionMapScopeCounts(fileRows);
...
test("checker rejects pending rows at handoff and allows them during batch phase", () => {
  ...
  const handoff = validateSurfaceFunctionMap({ repoRoot, runId, phase: "handoff" });
  assert.equal(hasFailure(handoff.results, "handoff-no-pending-surfaces"), true);

  const batch = validateSurfaceFunctionMap({ repoRoot, runId, phase: "batch" });
...
test("fill --next names the next pending upstream file without mutating rows", () => {
  ...
  const before = readJsonl(surfaceFunctionMapPathFor(repoRoot, runId)).rows;
  const output = runNode(surfaceFillScript, ["--repo", repoRoot, "--run-id", runId, "--next"], repoRoot);
  const payload = JSON.parse(output);
  const after = readJsonl(surfaceFunctionMapPathFor(repoRoot, runId)).rows;

  assert.equal(payload.schema, "foundation.backfill.surface-function-map-next-target.v1");
...
test("fill marks one full-read file at a time and checker passes handoff", () => {
  ...
  const surfaces = readJsonl(surfaceFunctionMapPathFor(repoRoot, runId)).rows;
...
test("fill can revise failed rows and records a revision event", () => {
  ...
  prepareArtifactInventory(repoRoot, runId);
  runNode(surfaceInitScript, ["--repo", repoRoot, "--run-id", runId], repoRoot);
  const surfacePath = surfaceFunctionMapPathFor(repoRoot, runId);
...
test("checker fails duplicate IDs, stale upstream hashes, and unresolved eligible files", () => {
  ...
  const results = validateSurfaceFunctionMap({ repoRoot, runId }).results;
...
`, "utf8");
  ...
  const results = validateSurfaceFunctionMap({ repoRoot, runId, phase: "batch" }).results;
  ...
  const handoff = validateSurfaceFunctionMap({ repoRoot, runId, phase: "handoff" }).results;
...
test("refresh invalidates surfaces when upstream Artifact Inventory rows change", () => {
  ...
  const runId = "20260527-01";
  prepareSurfaceFunctionMap(repoRoot, runId);
  const artifactInventoryPath = path.join(defaultBackfillDir(repoRoot), `artifact-inventory-${runId}.jsonl`);
  const fileRows = readJsonl(artifactInventoryPath).rows.map(row => (
    row.path === "web-app/backend/src/routes/dashboard.ts"
      ? { ...row, contentHash: "sha256:changed", sizeBytes: row.sizeBytes + 1 }
      : row
  ));
  writeJsonl(artifactInventoryPath, fileRows);
...
test("report command records handoff state and checker can detect report drift", () => {
  ...
  const runId = "20260527-01";
  const runLog = path.join("docs", "specs", "backfill", `run-log-${runId}.jsonl`);
  prepareSurfaceFunctionMap(repoRoot, runId, runLog);
  ...
  const drift = validateSurfaceFunctionMap({ repoRoot, runId, reportPath }).results;
...
test("surface check command writes check artifact", () => {
  ...
  assert.equal(check.schema, "foundation.backfill.surface-function-map-check.v1");
