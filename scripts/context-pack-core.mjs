...
const VALID_CONTEXT_PACK_STATUSES = new Set([
  "pending",
  "packed",
  "needs-evidence",
...
function isReadyForProcessMapStatus(status) {
  return status === READY_FOR_PROCESS_MAP_STATUS;
}
...
function contextPackPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `context-pack-${runId}.jsonl`);
}

function contextPackCheckPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `context-pack-check-${runId}.json`);
}

function contextPackEvalReceiptPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `context-pack-eval-${runId}.jsonl`);
}

function contextPackSummaryPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `context-pack-summary-${runId}.html`);
}

function contextPackRefreshPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `context-pack-refresh-${runId}.json`);
}

function contextPackArtifactFingerprint(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return fileFingerprint(contextPackPathFor(repoRoot, runId, outDir));
}
...
function stablePackId(sliceId, name, ordinal = 1) {
  ...
  return `ep-${slug(name || sliceId || "context-pack")}-${sha256Text(semantic).slice(0, 12)}`;
}
...
function readArtifactInventoryRows(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  const registryPath = registryPathFor(repoRoot, runId, outDir);
  const parsed = readJsonl(registryPath);
  return { registryPath, ...parsed };
}

function readContextPackRows(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
...
function validateSpecJobQueueHandoff(repoRoot, runId, outDir = defaultBackfillDir(repoRoot), reportPath = null) {
  const validation = validateSpecJobQueue({ repoRoot, runId, outDir, phase: "handoff", reportPath });
  const results = [...validation.results];

  const checkPath = specJobQueueCheckPathFor(repoRoot, runId, outDir);
  if (!fs.existsSync(checkPath)) {
    results.push(fail("upstream-spec-job-queue-check-artifact", "Passing Define Spec Jobs check artifact is required before Context Pack"));
  } else {
    const check = readJson(checkPath);
    results.push(check?.summary?.fail === 0
      ? pass("upstream-spec-job-queue-check-artifact", "Define Spec Jobs check artifact passes")
      : fail("upstream-spec-job-queue-check-artifact", "Define Spec Jobs check artifact must pass", { summary: check?.summary || null }));
  }

  const evalSummary = readEvalSummary(specJobQueueEvalReceiptPathFor(repoRoot, runId, outDir));
  const currentQueueFingerprint = specJobQueueArtifactFingerprint(repoRoot, runId, outDir);
  ...
  results.push(evalSummary?.acceptabilityGate?.acceptable && evalQueueFresh
    ? pass("upstream-spec-job-queue-eval", "Define Spec Jobs eval artifact passes and is current")
    : fail("upstream-spec-job-queue-eval", "Passing current Define Spec Jobs eval receipt is required before Context Pack", {
  ...
  results.push(revisionTargets.length === 0
    ? pass("upstream-spec-job-queue-eval-revisions", "Define Spec Jobs eval has no revision targets")
    : fail("upstream-spec-job-queue-eval-revisions", "Define Spec Jobs eval revision targets must be resolved before Context Pack", { revisionTargets }));

  const summaryPath = specJobQueueSummaryPathFor(repoRoot, runId, outDir);
  results.push(fs.existsSync(summaryPath)
    ? pass("upstream-spec-job-queue-eval-summary", "Define Spec Jobs HTML eval summary exists")
    : fail("upstream-spec-job-queue-eval-summary", "Define Spec Jobs HTML eval summary is required before Context Pack", { summaryPath: path.relative(repoRoot, summaryPath) }));

  if (reportPath) {
    if (!fs.existsSync(reportPath)) {
      results.push(fail("upstream-spec-job-queue-report-exists", "Define Spec Jobs report path does not exist", { reportPath }));
    } else {
      const state = parseJsonScript(fs.readFileSync(reportPath, "utf8"), "backfill-spec-job-queue-state");
      results.push(state?.nextLayer === "Context Pack"
        ? pass("upstream-spec-job-queue-report-handoff", "Define Spec Jobs report names Context Pack as next layer")
        : fail("upstream-spec-job-queue-report-handoff", "Define Spec Jobs report must name Context Pack as next layer", { nextLayer: state?.nextLayer || null }));
    }
  }

  return {
    queuePath: validation.queuePath,
    capabilityMapPath: validation.capabilityMapPath,
  ...
  return [
    {
      ...
      relationship: "spec-job-queue-row",
...
function createPendingContextPackRow(queueRow, queueFingerprint = null, ordinal = 1) {
  const now = nowIso();
...
function createInitialContextPackRows(queueRows, queueFingerprint = null) {
  return activeQueueRows(queueRows).map((row, index) => createPendingContextPackRow(row, queueFingerprint, index + 1));
}

function createAgentMarkedContextPackRow({ queueById, selectedSliceIds, spec, queueFingerprint, ordinal = 1 }) {
  ...
  const status = statusFromSpec && VALID_CONTEXT_PACK_STATUSES.has(statusFromSpec)
    ? statusFromSpec
...
function markContextPackRowsForSlices({ queueRows, packRows, sliceIds, packSpecs, queueFingerprint }) {
  ...
  const nextRows = packSpecs.map((spec, index) => createAgentMarkedContextPackRow({
    queueById,
    selectedSliceIds,
    spec,
    queueFingerprint,
  ...
  output.sort(compareContextPackRows);
  return {
    rows: output,
...
function compareContextPackRows(left, right) {
  const statusRank = {
    pending: 0,
    "needs-evidence": 1,
    packed: 2,
...
function nextContextPackTarget({ queueRows, packRows }) {
  const queueById = new Map(queueRows.map(row => [row.sliceId, row]));
...
function contextPackCoverageFindings(row) {
...
function validateContextPackRowShape(row, prefix, results, phase) {
  if (row?.schema !== "foundation.backfill.context-pack-row.v1") {
  ...
  if (!VALID_CONTEXT_PACK_STATUSES.has(row?.status)) {
...
function validateContextPackRows({ queueRows, capabilityRows, surfaceRows, fileRows, packRows, phase = "handoff" }) {
  const results = [];
  const queueById = new Map(queueRows.map(row => [row.sliceId, row]));
  ...
  results.push(validateUnique(packRows, "packId", "context-pack"));

  for (const [index, row] of packRows.entries()) {
    const prefix = `context-pack:${row?.packId || index + 1}`;
    validateContextPackRowShape(row, prefix, results, phase);
    ...
    if (isReadyForProcessMapStatus(row.status)) {
      for (const finding of contextPackCoverageFindings(row)) {
  ...
  results.push(stale.length === 0
    ? pass("context-pack-upstream-fresh", "Context Pack upstream slice fingerprints match Define Spec Jobs rows")
    : fail("context-pack-upstream-fresh", "Context Pack rows must be refreshed when upstream Define Spec Jobs rows change", { stale }));
  ...
  results.push(duplicateSlicePacks.length === 0
    ? pass("context-pack-one-row-per-slice", "Each queued slice has at most one Context Pack row")
    : fail("context-pack-one-row-per-slice", "Each queued slice must have at most one Context Pack row", { duplicateSlicePacks }));
  ...
  if (uncovered.length === 0) {
    results.push(pass("context-pack-covers-active-slices", "Every active Define Spec Jobs slice has Context Pack coverage"));
  } else if (phase === "handoff") {
    results.push(fail("context-pack-covers-active-slices", "Context Pack must cover every active Define Spec Jobs slice before Process / Action Map", { uncovered }));
  } else {
    results.push(warn("context-pack-covers-active-slices", `${uncovered.length} active queue slice(s) still need Context Pack coverage`, { uncovered }));
  }
...
function readContextPackEvalSummary(repoRoot, runId, outDir) {
  return readEvalSummary(contextPackEvalReceiptPathFor(repoRoot, runId, outDir));
}

function validateContextPackEvalFreshness({ repoRoot, runId, outDir, packRows, phase = "handoff" }) {
  const evalSummary = readContextPackEvalSummary(repoRoot, runId, outDir);
  if (!evalSummary) return [];
  const currentFingerprint = contextPackArtifactFingerprint(repoRoot, runId, outDir);
  ...
  const results = fresh
    ? [pass("context-pack-eval-current", "Context Pack eval receipt matches the current pack artifact")]
    : [fail("context-pack-eval-current", "Context Pack eval must be regenerated after pack artifact changes", {
  ...
  if (phase === "handoff") {
    const revisionTargets = Array.isArray(evalSummary.revisionTargets) ? evalSummary.revisionTargets : [];
    results.push(revisionTargets.length === 0
      ? pass("context-pack-eval-revisions", "Context Pack eval has no revision targets")
      : fail("context-pack-eval-revisions", "Context Pack eval revision targets must be resolved before Process / Action Map", { revisionTargets }));
  }
  return results;
}

function validateContextPackReportState({ repoRoot, runId, outDir, reportPath, queueRows, capabilityRows, surfaceRows, fileRows, packRows }) {
  if (!reportPath) return [];
  if (!fs.existsSync(reportPath)) return [fail("context-pack-report-exists", "Report path passed to checker does not exist", { reportPath })];
  const html = fs.readFileSync(reportPath, "utf8");
  const state = parseJsonScript(html, "backfill-context-pack-state");
  if (!state) return [fail("context-pack-report-state", "Report is missing backfill-context-pack-state JSON script")];
  const expected = buildContextPackReportState({ repoRoot, runId, outDir, queueRows, capabilityRows, surfaceRows, fileRows, packRows });
  const drift = [];
  ...
  return drift.length === 0
    ? [pass("context-pack-report-state-current", "Context Pack report state matches canonical artifacts")]
    : [fail("context-pack-report-state-current", "Context Pack report state must match canonical artifacts", { drift })];
}

function validateContextPack({ repoRoot, runId, outDir = defaultBackfillDir(repoRoot), phase = "handoff", reportPath = null, skipEvalFreshness = false }) {
  const upstream = validateSpecJobQueueHandoff(repoRoot, runId, outDir);
  ...
  for (const [label, parsed] of [
    ...
    ["surface-function-map", surfaceFunctionMap],
    ["capability-map", capabilityMap]
  ]) {
    if (parsed.errors.length > 0) {
      results.push(...parsed.errors.map(error => fail(`context-pack-upstream-${label}-jsonl:${error.line}`, `${label} JSONL line must parse`, error)));
    }
  }
  ...
  if (!fs.existsSync(packPath)) {
    return {
      ...
      packRows: [],
      results: [...results, fail("context-pack-exists", `Context Pack artifact does not exist: ${packPath}`)]
    };
  }
  const parsed = readJsonl(packPath);
  results.push(pass("context-pack-exists", "Context Pack artifact exists"));
  if (parsed.errors.length > 0) {
    results.push(...parsed.errors.map(error => fail(`context-pack-jsonl:${error.line}`, "Context Pack JSONL line must parse", error)));
    return {
      packPath,
      queuePath: upstream.queuePath,
      queueRows: upstream.queueRows,
  ...
  results.push(pass("context-pack-jsonl", "Every Context Pack line parses as JSON"));
  results.push(...validateContextPackRows({
    queueRows: upstream.queueRows,
    capabilityRows: capabilityMap.rows,
    surfaceRows: surfaceFunctionMap.rows,
    fileRows: artifactInventory.rows,
  ...
  if (!skipEvalFreshness) {
    results.push(...validateContextPackEvalFreshness({ repoRoot, runId, outDir, packRows: parsed.rows, phase }));
  }
  results.push(...validateContextPackReportState({
    repoRoot,
    runId,
    outDir,
    reportPath,
...
function selectContextPackEvalSample(packRows, mode = "risk") {
...
function scoreContextPackRow(row, queueById = new Map()) {
  const findings = [];
  ...
  const coverageFindings = contextPackCoverageFindings(row);
...
function aggregateContextPackEval(checkResults, rowReceipts) {
...
function mergeContextPackRowsForRefresh({ queueRows, existingPackRows, queueFingerprint = null }) {
  ...
  for (const row of existingPackRows) {
    ...
    if (stale) {
      ...
      output.push(createPendingContextPackRow(queueRow, queueFingerprint));
      continue;
    }
...
function buildContextPackPayload({ runId, repoRoot, packRows }) {
  return {
    schema: "foundation.backfill.context-pack.v1",
    runId,
...
function buildContextPackReportState({ repoRoot, runId, outDir, queueRows, capabilityRows, surfaceRows, fileRows, packRows, runLogPath = null }) {
  const checkPath = contextPackCheckPathFor(repoRoot, runId, outDir);
  const check = fs.existsSync(checkPath) ? readJson(checkPath) : null;
  const evalReceiptPath = contextPackEvalReceiptPathFor(repoRoot, runId, outDir);
  const evalSummary = readContextPackEvalSummary(repoRoot, runId, outDir);
  ...
  return {
    schema: "foundation.backfill.context-pack-report-state.v1",
    runId,
    generatedAt: new Date().toISOString(),
    packPath: path.relative(repoRoot, contextPackPathFor(repoRoot, runId, outDir)),
    packFingerprint,
    ...
    summaryPath: path.relative(repoRoot, contextPackSummaryPathFor(repoRoot, runId, outDir)),
...
export {
  ...
  VALID_CONTEXT_PACK_STATUSES,
  appendRunLogEvent,
  aggregateContextPackEval,
  buildContextPackPayload,
  buildContextPackReportState,
  compareContextPackRows,
  createAgentMarkedContextPackRow,
  createInitialContextPackRows,
  createPendingContextPackRow,
  defaultBackfillDir,
  ensureDir,
