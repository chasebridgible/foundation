...
function usage() {
  return `Usage:
  npm run foundation:capability-map:fill -- --repo /path/to/repo --run-id YYYYMMDD-NN --next [--out-dir path]
  npm run foundation:capability-map:fill -- --repo /path/to/repo --run-id YYYYMMDD-NN --surface-ids surface:a,surface:b --capabilities-json '[...]' [--run-log path]
...
function main() {
  ...
  const capabilityMap = readJsonl(matrixPath);
  ...
  if (options.next) {
    console.log(JSON.stringify({
      schema: "foundation.backfill.capability-map-next-target.v1",
      runId,
      target: nextCapabilityMapTarget({
