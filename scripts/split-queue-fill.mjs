...
Creates reviewed queue slices from Capability Matrix rows. Generated slice files, --all, and --batch-size shortcuts are rejected.`;
}

function readSliceSpecs(options) {
  if (options["slices-file"]) {
    throw new Error("Split And Queue fill does not accept --slices-file; pass reviewed --slices-json inline");
  }
...
function main() {
  ...
  if (options.all || options["batch-size"]) {
    throw new Error("Split And Queue fill does not support --all or --batch-size; select reviewed capabilities explicitly with --capability-ids");
  }
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    summary: marked.revisionCount > 0
      ? `Revised Split And Queue rows for ${marked.markedCapabilityIds.length} capability row(s).`
      : `Marked Split And Queue rows for ${marked.markedCapabilityIds.length} capability row(s).`,
    ...
    nextAction: pendingCount > 0 || inProgressCount > 0
      ...
      : "Run Split And Queue checker and eval."
  });

  console.log(`Split And Queue fill
