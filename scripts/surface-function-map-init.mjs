...
function usage() {
  return `Usage:
  npm run foundation:surface-function-map:init -- --repo /path/to/repo --run-id YYYYMMDD-NN [--out-dir path] [--run-log path]
...
function main() {
  ...
  const handoff = validateArtifactInventoryHandoff(repoRoot, runId, outDir);
  ...
  const scope = surfaceFunctionMapScopeCounts(handoff.registry.rows);
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    commands: ["foundation:surface-function-map:init"],
    checks: [{ name: "artifact-inventory-handoff", result: "passed" }],
    durationSeconds: 0,
  ...
  console.log(renderResultsText("Surface / Function Map init", [
    ...handoff.results,
    { id: "surface-function-map-skeleton", status: "pass", message: `Created ${rows.length} pending Surface / Function Map-eligible row(s); skipped ${scope.skipped.length} inert file row(s)` }
  ]));
}
