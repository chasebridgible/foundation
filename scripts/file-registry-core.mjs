...
function currentRunArtifactPaths(runId) {
  return new Set([
    ...
    `docs/specs/backfill/file-registry-${runId}.jsonl`,
    `docs/specs/backfill/file-registry-check-${runId}.json`,
    `docs/specs/backfill/file-registry-eval-${runId}.jsonl`,
    `docs/specs/backfill/file-registry-eval-summary-${runId}.html`,
    `docs/specs/backfill/file-registry-refresh-${runId}.json`,
    `docs/specs/backfill/file-registry-handoff-${runId}.html`,
...
function validateRegistryRows({ repoRoot, manifest, rows, phase = "handoff", mode = "bootstrap" }) {
  ...
  for (const [index, row] of rows.entries()) {
    ...
    if (row?.schema !== "foundation.backfill.file-registry-row.v1") {
...
function loadCapabilityMatrix(repoRoot) {
  ...
  for (const report of reports) {
    ...
    const matrix = parseJsonScript(html, "backfill-capability-matrix");
...
function validateGraphLinks({ repoRoot, rows, strict = false }) {
  ...
  const capabilityMatrix = loadCapabilityMatrix(repoRoot);
  const specIds = new Set((specRegistry?.specs || []).map(spec => spec.id));
  const sectionRefs = specSectionSet(specRegistry);
  const capabilityIds = new Set((capabilityMatrix?.capabilities || []).map(capability => capability.id));
...
function inferKind(filePath, extension) {
  ...
  if (lower.includes("docs/specs/backfill/file-registry") || lower.includes("docs/specs/backfill/file-manifest")) return "generated";
...
function inferSpecLinks(repoRoot, row) {
  ...
  const candidatesByDomain = {
    ...
    "backfill-process": [["sandia.backfill.20260521-01.report", "file-registry-checkpoint", "documents"]],
  ...
  return candidates
    ...
    .map(([specId, sectionId, relationship]) => ({
      ...
      sectionId,
      relationship,
      evidence: `Inferred from file-registry domain ${row.domain} and path ${row.path}.`
    }));
}

function inferCapabilityIds(repoRoot, row) {
  const matrix = loadCapabilityMatrix(repoRoot);
  const existing = new Set((matrix?.capabilities || []).map(capability => capability.id));
  const byDomain = {
    "web-app": "cap-web-app-user-flows",
