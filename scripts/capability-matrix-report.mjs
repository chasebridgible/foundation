...
function usage() {
  return `Usage:
  npm run foundation:capability-matrix:report -- --repo /path/to/repo --run-id YYYYMMDD-NN [--report docs/specs/backfill/review-report-YYYYMMDD-NN.html] [--run-log path]

Creates or updates a target backfill report with Capability Matrix V1 handoff state and embedded backfill-capability-matrix JSON.`;
}
...
function buildState({ repoRoot, runId, outDir, runLogPath }) {
  const registryPath = capabilityMatrixPathFor(repoRoot, runId, outDir);
...
function renderSection(state) {
  return `<section id="capability-matrix-v1-state" data-spec-section="capability-matrix-v1-state" data-section-type="handoff">
  <h2>Capability Matrix V1 State</h2>
  ...
  <p>Split And Queue may only consume this layer when the next layer is <code>split and queue</code>; otherwise rows named by check/eval receipts must return to the Capability Matrix fill loop.</p>
</section>`;
}
...
function renderCapabilityRows(matrixPayload) {
  ...
  return `<section id="capability-matrix-v1-table" data-spec-section="capability-matrix-v1-table" data-section-type="state">
  <h2>Capability Matrix Rows</h2>
  <table class="status-table">
...
<title>Capability Matrix Report ${state.runId}</title>
<link rel="stylesheet" href="../spec-system.css">
<script type="application/json" id="backfill-capability-matrix-state">
...
<main class="main">
  <section id="summary">
    ...
    <p class="lede">Minimal report generated by the Foundation Capability Matrix V1 report integration.</p>
  </section>
...
function upsertReportState(html, state, matrixPayload) {
  const stateScript = `<script type="application/json" id="backfill-capability-matrix-state">\n${JSON.stringify(state, null, 2)}\n</script>`;
  const matrixScript = `<script type="application/json" id="backfill-capability-matrix">\n${JSON.stringify(matrixPayload, null, 2)}\n</script>`;
  const section = `${renderSection(state)}\n${renderCapabilityRows(matrixPayload)}`;
  let nextHtml = html;
  if (/<script\b[^>]*id=["']backfill-capability-matrix-state["'][^>]*>[\s\S]*?<\/script>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<script\b[^>]*id=["']backfill-capability-matrix-state["'][^>]*>[\s\S]*?<\/script>/i, stateScript);
  } else {
    nextHtml = nextHtml.replace("</head>", `${stateScript}\n</head>`);
  }

  if (/<script\b[^>]*id=["']backfill-capability-matrix["'][^>]*>[\s\S]*?<\/script>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<script\b[^>]*id=["']backfill-capability-matrix["'][^>]*>[\s\S]*?<\/script>/i, matrixScript);
  } else {
    nextHtml = nextHtml.replace("</head>", `${matrixScript}\n</head>`);
  }

  if (/<section\b[^>]*id=["']capability-matrix-v1-state["'][^>]*>[\s\S]*?<\/section>\s*<section\b[^>]*id=["']capability-matrix-v1-table["'][^>]*>[\s\S]*?<\/section>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<section\b[^>]*id=["']capability-matrix-v1-state["'][^>]*>[\s\S]*?<\/section>\s*<section\b[^>]*id=["']capability-matrix-v1-table["'][^>]*>[\s\S]*?<\/section>/i, section);
  } else if (/<section\b[^>]*id=["']capability-matrix-v1-state["'][^>]*>[\s\S]*?<\/section>/i.test(nextHtml)) {
    nextHtml = nextHtml.replace(/<section\b[^>]*id=["']capability-matrix-v1-state["'][^>]*>[\s\S]*?<\/section>/i, section);
...
function main() {
  ...
  let matrixRows = readJsonl(capabilityMatrixPathFor(repoRoot, runId, outDir)).rows;
  let matrixPayload = buildCapabilityMatrixPayload({ runId, repoRoot, capabilityRows: matrixRows });
  ...
  appendRunLogEvent(runLogPath, {
    ...
    phase: "handoff",
    event: "handoff",
    summary: `Capability Matrix handoff recorded with next layer ${state.nextLayer}.`,
    artifactsRead: [
      state.registryPath,
      state.checkerPath,
    ...
    commands: ["foundation:capability-matrix:report"],
    checks: [{ name: "capability-matrix-handoff", result: state.nextLayer === "split and queue" ? "passed" : "needs-revision" }],
    result: state.nextLayer === "split and queue"
      ? "Capability Matrix handoff gate passed."
      : "Capability Matrix handoff gate did not pass.",
    nextAction: state.nextLayer === "split and queue"
      ? "Split And Queue may consume Capability Matrix artifacts."
      : "Revise Capability Matrix rows named by checker or eval receipts."
  });
  state = buildState({ repoRoot, runId, outDir, runLogPath });
  matrixRows = readJsonl(capabilityMatrixPathFor(repoRoot, runId, outDir)).rows;
  matrixPayload = buildCapabilityMatrixPayload({ runId, repoRoot, capabilityRows: matrixRows });
