...
function usage() {
  return `Usage:
  npm run foundation:surface-function-map:fill -- --repo /path/to/repo --run-id YYYYMMDD-NN --next [--out-dir path]
  npm run foundation:surface-function-map:fill -- --repo /path/to/repo --run-id YYYYMMDD-NN --path repo/relative/file --surfaces-json '[...]' [--run-log path]
...
function main() {
  ...
  if (options.next) {
    console.log(JSON.stringify({
      schema: "foundation.backfill.surface-function-map-next-target.v1",
      runId,
      target: nextSurfaceFunctionMapTarget({
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    artifactsRead: [path.relative(repoRoot, artifactInventory.registryPath), path.relative(repoRoot, surfacePath), marked.markedPath],
