...
Creates or updates a target backfill report with the Surface Registry V1 handoff state.`;
}
...
function buildState({ repoRoot, runId, outDir, runLogPath }) {
  ...
  return {
    ...
    nextLayer: pendingCount === 0 && needsEvidenceCount === 0 && blockingFlagCount === 0 && checkerPass && evalPass
      ? "capability matrix"
...
function renderSection(state) {
  ...
  <h2>Surface Registry V1 State</h2>
  ...
  <p>Capability Matrix may only consume this layer when the next layer is <code>capability matrix</code>; otherwise rows named by check/eval receipts must return to the Surface Registry fill loop.</p>
</section>`;
}
...
<title>Surface Registry Report ${state.runId}</title>
...
<main class="main">
  <section id="summary">
    ...
    <p class="lede">Minimal report generated by the Foundation Surface Registry V1 report integration.</p>
  </section>
...
function main() {
  ...
  appendRunLogEvent(runLogPath, {
    ...
    phase: "handoff",
    event: "handoff",
    summary: `Surface Registry handoff recorded with next layer ${state.nextLayer}.`,
    artifactsRead: [
      state.registryPath,
      state.checkerPath,
    ...
    checks: [{ name: "surface-registry-handoff", result: state.nextLayer === "capability matrix" ? "passed" : "needs-revision" }],
    result: state.nextLayer === "capability matrix"
      ? "Surface Registry handoff gate passed."
      : "Surface Registry handoff gate did not pass.",
    nextAction: state.nextLayer === "capability matrix"
      ? "Capability Matrix may consume Surface Registry artifacts."
      : "Revise Surface Registry rows named by checker or eval receipts."
  });
