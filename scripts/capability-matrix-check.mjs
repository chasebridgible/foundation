...
Validates Capability Matrix structure, upstream Surface Registry references, freshness, coverage, handoff states, split flags, and optional report state.`;
}
...
function main() {
  ...
  appendRunLogEvent(options["run-log"] ? path.resolve(repoRoot, options["run-log"]) : null, {
    ...
    phase: "validation",
    event: "validation",
    summary: `Capability Matrix check ${summary.fail === 0 ? "passed" : "failed"}.`,
    ...
    result: summary.fail === 0 ? "Capability Matrix check passed." : "Capability Matrix check failed.",
    nextAction: summary.fail === 0 ? "Run Capability Matrix eval or record handoff." : "Fix Capability Matrix check failures."
  });

  if (options.json) {
    console.log(JSON.stringify(payload, null, 2));
  } else {
    console.log(renderResultsText("Capability Matrix check", check.results));
  }
