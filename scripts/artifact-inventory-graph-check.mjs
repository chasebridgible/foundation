...
function usage() {
  return `Usage:
  npm run foundation:artifact-inventory:graph-check -- --repo /path/to/repo --run-id YYYYMMDD-NN [--mode bootstrap|strict] [--json]
...
function main() {
  ...
  if (options.json) {
    console.log(JSON.stringify({ schema: "foundation.backfill.artifact-inventory-graph-check.v1", runId, summary, results }, null, 2));
  } else {
