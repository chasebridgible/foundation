...
function capabilityMatrixPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `capability-matrix-${runId}.jsonl`);
}

function capabilityCheckPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `capability-matrix-check-${runId}.json`);
}

function capabilityEvalReceiptPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `capability-matrix-eval-${runId}.jsonl`);
}

function capabilitySummaryPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `capability-matrix-summary-${runId}.html`);
}

function capabilityRefreshPathFor(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
  return path.join(outDir, `capability-matrix-refresh-${runId}.json`);
}
...
function createPendingCapabilityRow(surfaceRow) {
  ...
  return {
    schema: "foundation.backfill.capability-matrix-row.v1",
...
function readSurfaceRegistryRows(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
...
function readCapabilityMatrixRows(repoRoot, runId, outDir = defaultBackfillDir(repoRoot)) {
...
function validateSurfaceRegistryHandoff(repoRoot, runId, outDir = defaultBackfillDir(repoRoot), reportPath = null) {
  const validation = validateSurfaceRegistry({ repoRoot, runId, outDir, phase: "handoff", reportPath });
  ...
  return {
    surfaceRegistryPath: validation.registryPath,
...
function nextCapabilityMatrixTarget({ surfaceRows, capabilityRows }) {
  const surfaceById = new Map(surfaceRows.map(row => [row.surfaceId, row]));
...
function validateCapabilityRowShape(row, prefix, results) {
  if (row?.schema !== "foundation.backfill.capability-matrix-row.v1") {
...
function validateCapabilityMatrix({ repoRoot, runId, outDir = defaultBackfillDir(repoRoot), phase = "handoff", reportPath = null }) {
  const upstream = validateSurfaceRegistryHandoff(repoRoot, runId, outDir);
...
function buildCapabilityMatrixPayload({ runId, repoRoot, capabilityRows }) {
  return {
    schema: "foundation.backfill.capability-matrix.v1",
    runId,
