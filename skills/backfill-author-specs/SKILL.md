...
8. If check or eval returns any failure, warning, missing outstanding receipt, or revision target, revise that same target and repeat steps 5-7.
...
- Do not hand off with stale check/eval artifacts, report drift, placeholders, generic evidence, missing graph metadata, missing job/technical spec paths, or under-reviewed rows.
