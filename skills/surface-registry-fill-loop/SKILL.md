...
5. Run the batch checker often enough that structure failures are fixed before many more files are marked.
6. Repeat until no pending or failed eligible rows remain.
7. Run handoff check, eval, and report once the layer is terminal.
...
- Route files split by durable endpoint or handler when the file exposes more than one API or route boundary.
...
- Evidence stays short: state the full-file-read basis for the decision.
