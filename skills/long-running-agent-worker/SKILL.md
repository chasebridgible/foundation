...
description: Run a long-running Codex worker task that must keep making useful, source-backed progress until a time gate or other durable exit criterion is satisfied. Use for timed research passes, broad repo backfills, recurring scout-style runs, automation dry runs, or any agent task where early self-declared completion, idling, clock-watching, context loss, or weak handoff would make the result unreliable.
---
...
Treat the timer or long-run bound as an exit gate, not a work strategy. The worker should continue useful bounded work until the gate allows exit, while preserving enough durable state that another agent can resume without chat memory.
...
5. Start or record the time gate immediately when the task asks for one. Persist `startedAt`, `deadlineAt`, `duration`, and the command or mechanism used.
...
- Do not use `sleep`, long waits, timer guard sessions, or idle holding to pass time.
- Do not check the timer before each unit.
- Check the timer only at a real exit or handoff gate after a substantial work block.
- A good default threshold is both: at least 30 minutes since the previous timer check, and at least 12 bounded records, validations, or source-family attempts since that check.
...
- When writing reports or briefs, include timer status only for the current run ID. Keep prior timer receipts as history, not as current-run evidence.

Short timer checks are acceptable only near the actual deadline or when an external blocker genuinely prevents further useful work. A repeated timer check without intervening evidence-backed work is a workflow failure.

## Forbidden Shortcuts

- Do not write a broad "source exhaustion" note and then stop while the timer remains open.
...
1. Read the goal, branch, run ID, timer state, status, latest receipts, and uncommitted files.
...
- the final handoff names run ID, branch, commit or blocker, timer start/deadline/final check, timer-check count, useful work completed, files changed, validations, unresolved risks, and exact continuation point.
